...
# Add confirmModalCancelBtn support
old_btn = """                btn.onclick = () => {
                    closeConfirmModal();
                    onConfirmCallback();
                };"""
...
# Replace the submission handler body to wrap it in processSubmit()
start_idx = content.find("campaignForm.onsubmit = async (e) => {")
end_idx = content.find("        function toggleTemplateFields(show) {", start_idx)

# We want to replace the `campaignForm.onsubmit` block.
old_submit_block = content[start_idx:end_idx]

# Let's just do it directly via string replace if we can find it
if "const submitBtn = document.getElementById('launchBtn');" in old_submit_block:

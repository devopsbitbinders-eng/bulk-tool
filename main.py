...
async def get_dashboard_stats(user_id: int):
    ...
    if is_mysql:
        ...
        out_records = await db.fetch_all("SELECT timestamp, CASE WHEN clicked=1 THEN 'clicked' ELSE status END as status FROM messages WHERE user_id = :u AND timestamp >= :ts", {"u": user_id, "ts": seven_days_str})
...
async def upload_file(
    ...
    return {"message": "Campaign queued", "campaign_id": campaign_id, "total": len(data), "removed_opt_outs": removed_opt_outs, "filename": filename}
            ...
            for change in entry.get("changes", []):
                ...
                for echo_data in all_echoes:
                    try:
                        ...
                        print(f"DEBUG WEBHOOK: Error processing echo message: {echo_err}")
                        print(f"DEBUG WEBHOOK ECHO PAYLOAD: {json.dumps(echo_data)}")

...
nav_pattern = r'    <!-- Navigation -->\s*<nav class="bg-white border-b border-slate-200 pt-4 mb-4 flex flex-col gap-4">.*?</nav>'

new_nav = """    <!-- Navigation -->
    <nav class="bg-white border-b border-slate-200 px-8 h-20 mb-4">
        <div class="max-w-7xl mx-auto flex justify-between items-center w-full h-full">
            ...
            <div class="flex items-center gap-6 h-full">
                ...
                <div class="flex items-center gap-2 border-l border-slate-200 pl-6 h-10">
                    ...
                    <button onclick="openManualAuthModal()"
                        class="p-2 text-slate-400 hover:text-indigo-600 transition-colors bg-white rounded-xl border border-slate-100 shadow-sm h-full"
                    ...
                    <div class="flex items-center gap-1 h-full">
                        <button onclick="unlinkAccount()"
                            class="p-2 text-slate-400 hover:text-amber-500 transition-colors bg-slate-50 rounded-xl border border-slate-100 h-full"
            ...
            <!-- CENTER: Tabs -->
            <div class="flex items-center gap-2 h-full relative top-[1px]">
                <button onclick="switchTab('campaigns')" id="tab-campaigns"
                    class="tab-btn active h-full px-4 font-medium text-sm transition-all text-slate-500 hover:text-slate-800 flex items-center border-b-2 border-transparent">Campaigns</button>
                <button onclick="switchTab('analytics')" id="tab-analytics"
                    class="tab-btn h-full px-4 font-medium text-sm text-slate-500 hover:text-slate-800 transition-all flex items-center border-b-2 border-transparent">Analytics</button>
                <button onclick="switchTab('templates')" id="tab-templates"
                    class="tab-btn h-full px-4 font-medium text-sm text-slate-500 hover:text-slate-800 transition-all flex items-center border-b-2 border-transparent">Templates</button>
                <button onclick="switchTab('history')" id="tab-history"
                    class="tab-btn h-full px-4 font-medium text-sm text-slate-500 hover:text-slate-800 transition-all flex items-center border-b-2 border-transparent">Sending History</button>
                <button onclick="switchTab('chat')" id="tab-chat"
                    class="tab-btn h-full px-4 font-medium text-sm text-slate-500 hover:text-slate-800 transition-all flex items-center gap-1.5 border-b-2 border-transparent">
                    Chat
            ...
            <div class="flex items-center gap-4 h-full">
                ...
                <div class="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div class="flex flex-col">
                        ...
                        {% if linked_phone %}
                        <span class="text-[8px] text-slate-400 font-medium leading-none mt-0.5">WABA: {{ waba_id }}</span>
                        {% endif %}
                    </div>
                </div>

                <!-- Profile Dropdown -->
                <div class="relative flex items-center h-full" id="profile-container">
                    ...
                    <!-- Dropdown Menu -->
                    <div id="profileMenu"
                        class="hidden absolute right-0 top-16 w-56 bg-white rounded-2xl border border-slate-100 shadow-2xl z-[5000] overflow-hidden">
